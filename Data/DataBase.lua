...
local function UseSet(data)
	...
	--Filter by addon.armorTypeFilter (our own dropdown), not Blizzard's unsynced class filter.
	...
	if data.classMask and (data.classMask == 0 or data.classMask == 16383) then		correctClass = true;
		...
		if data.setType == "Blizzard" then
			...
			if not addon.Profile.IgnoreClassRestrictions then
				...
				--Extra sets need the same nil-classMask fallback as the Blizzard-sets branch above.
...
function addon:GetCollectedExtraSetCount()
	...
	local function loadNarcissusSharedSets()
		...
		for i, setInfo in ipairs(NarciTransmogUIDB.SharedSets) do
			local itemTransmogInfoList = setInfo.cmd and TransmogUtil.ParseCustomSetSlashCommand(setInfo.cmd)
	...
	function addon:SaveNarcissusSharedSet(name, itemTransmogInfoList)
		...
		if #NarciTransmogUIDB.SharedSets >= MAX_NARCISSUS_SHARED_SETS then return false end

		local cmd = TransmogUtil.CreateCustomSetSlashCommand(itemTransmogInfoList);
		...
		table.insert(NarciTransmogUIDB.SharedSets, {
			...
			timeModified = time(),
			classID = classID,
			dataIndex = #NarciTransmogUIDB.SharedSets + 1,
		});
...
--[[
	...
	--Profile key for "Shared Sets", backed by Narcissus's shared-set pool (see loadNarcissusSharedSets above).
