...
addon.db_variantsid = {}
addon.db_baselist = {}
addon.unavailableSets = {
	[3443] = true,
}

local BaseSetsData = {}
addon.BaseSetsData = BaseSetsData
local BaseListLabels = {}
addon.BaseListLabels = BaseListLabels
local BaseList = {}
addon.BaseList = BaseList
local BaseSets = {}
addon.BaseSets = BaseSets
local VariantSets = {};
addon.VariantSets = VariantSets
local VariantSetsIDs = {};
addon.VariantSetsIDs = VariantSetsIDs
local ExtraSets = {}
addon.ExtraSets = ExtraSets 
local IgnoredSets = {};
addon.IgnoredSets = IgnoredSets
local NotUsedSets = {}
addon.NotUsedSets = NotUsedSets;
addon.BaseSetsPvPFlag = {}
...
function BuildBlizzSets()
	...
	local armorType = addon.selectedArmorType or addon.Globals.CLASS_INFO[playerClass][3]
	...
	for index, data in ipairs(sets) do
		if data.name ~= "PH" then
			...
			---if BetterWardrobeSetsDataProviderMixin:ValidSet(data) then


				if addon.MiscSets.customGroups and addon.MiscSets.customGroups[data.setID] then
					data.customGroups = addon.MiscSets.customGroups[data.setID];
				end
				...
				--if addon.altLabelDB[data.expansionID] and addon.altLabelDB[data.expansionID][data.setID] then
				 -- data.label = addon.altLabelDB[data.expansionID][data.setID];
				-- elseif addon.altLabelAppendDB[data.expansionID] and addon.altLabelAppendDB[data.expansionID][data.setID] then
					--data.label = data.label..addon.altLabelAppendDB[data.expansionID][data.setID];
				 -- end
				...
				--Shop & Trading Post
				elseif data.description == inGameShopGlobalString then -- or addon.MiscSets.SHOP_SETS[data.setID] or data.label == tradingPostGlobalString or addon.MiscSets.TRADINGPOST_SETS[data.setID] or (data.description and string.find(data.description, tradingPostGlobalString)) then 
					--data.tab = 3
					data.filter = 12
					data.tp = true
				else
				...
				--local sources = addon:GetSetSources(data.setID, data);
				--addon:AddSetSource(data.setID, sources)

				if data.classMask == 0 and addon.MiscSets.SPECIAL_SETS[data.setID] then
						...
						if not initSpecialSet then
							...
							if data.favorite then
								--local baseSet = BetterWardrobeSetsDataProviderMixin:GetSetByID(initSpecialSet);
				...
				elseif data.label == tradingPostGlobalString  then -- or addon.MiscSets.TRADINGPOST_SETS[data.setID] then
					--data.tab = 3
					data.filter = 12
				 ...
				 ---- BaseSetsPvPFlag[data.setID] = pvpDescriptions[data.description] or false;
					...
					--if(( data.name and BaseListNames[data.name] )and (data.label and BaseListLabels[data.label])) then

						--if data.name == BaseListNames[data.name]  then
						--subSet = true;
						--subSetBaseID = BaseListNames[data.name]
					--end
					
					if subSet then
						if data.favorite then
							----if not BaseSets[subSetBaseID].favoriteSetID then
							----	BaseSets[subSetBaseID].favoriteSetID = data.setID;
							----end
						end
				...
				--Set unavailable sets
				if data.setID <= 1446 and data.setID >= 1436  or data.label == "Pandaria Challenge Dungeons" then
					data.unavailable = true;

				elseif (data.description == ELITE) and not data.limitedTimeSet then
					data.unavailable = true;

			--	elseif addon.MiscSets.UNAVAILABLE_SETS[data.setID] then
				--	data.unavailable = true;
				end
...
-- for i,callback in pairs(ExpandedCallbacks)do
 --  if i==-1 or ExS_Settings.expansionToggles[i+1] then
	 ...
	 -- end
 -- end
	local function testSourceInfo(setID)
	local sources = self:GetSetSources(setID);
	for sourceID,_ in pairs(sources) do
		local sourceInfo = C_TransmogCollection.GetSourceInfo(sourceID);
		if sourceInfo == nil then
			local set = self:GetSetByID(setID);
		end
	end
end
	...
	----BetterWardrobeSetsDataProviderMixin:SortSets(BaseList, false);
end	
...
--[[
	...
	for i, data in ipairs(allSets) do
		if not (data.name == "PH") then
			...
			if data.baseSetID then
				--print("inst")
				if data.baseSetID == data.setID then 
					--print("inst")
					--tinsert(addon.db_basesets,data)
				end

				addon.db_basesets[data.setID] = data;
				table.insert(addon.db_baselist,data);



				data.baseSetID = data.setID;
				addon.db_variants[data.setID] = addon.db_variants[data.setID] or {}
				tinsert(addon.db_variants[data.setID], data)

				addon.db_variantsid[data.setID] = data.setID;

						--addon.db_variants[data.baseSetID] = addon.db_variants[data.baseSetID] or {}
						--tinsert(addon.db_variants[data.baseSetID],data.setID)
...
do
	local function BuildArmorDB(force)
		 BetterWardrobeSetsDataProviderMixin:ClearSets()
		--print(addon.selectedArmorType)
		--if (force == false) and armorType and addon.selectedArmorType then return; end
		...
		----for armorType, data in pairs(addon.ArmorSets) do\
			armorType = addon.selectedArmorType or addon.Globals.CLASS_INFO[playerClass][3]
			ArmorDB[armorType] = {}
			local armorSetdata = {addon.ArmorSets[armorType], addon.ArmorSets["COSMETIC"]}

		for i, armorTypeSetdata in ipairs(armorSetdata ) do

			for id, setData in pairs(armorTypeSetdata) do
				...
				if (setData.requiredFaction and setData.requiredFaction == GetFactionID(playerFaction) or setData.requiredFaction == nil) and 
					(not armorSetdata.BuildBlizzSets and (setData.filter ~= 5 and setData.filter ~= 7 and setData.filter ~= 11)) then 
					--setData.isHeritageArmor = string.find(setData.name, "Heritage")

 					--if BetterWardrobeSetsDataProviderMixin:ValidSet(setData) then
					...
					for i, set_Data in pairs(setData.itemData) do
					end
				...
				-----data.expansionID = expansionID;
					...
					for slotID, itemData in pairs(setData.itemData) do
						...
						if appearanceID  then --and setData.sources[item] and setData.sources[item] ~= 0 then 
							...
							ItemDB[appearanceID] = ItemDB[appearanceID] or {}
							ItemDB[appearanceID][newID] = setData
							setData.sources[itemData[2]] = false

						end
					end
					

			-----		addon:AddSetSource(setData.setID, setData.sources)
					...
					----setData.favorite = IsFavorite(setData.setID);
					if setData.favorite then
					----	--setData.favoriteSetID = setData.setID;
					end

					if setData.description == "" or setData.description == nil then setData.description = setData.name; end
					
					if addon.Globals.CLASS_NAMES[setData.classMask] ~= nil then
						----setData.description = addon:GetClassColor(setData.classMask, setData.description);
					end
					
					local subSet = false;
					local subSetBaseID = 0;
					

					setData.customGroups = setData.label.."-"..setData.armorType

					if setData.customGroups and BaseListLabels[setData.customGroups]  then
						subSet = true;
						subSetBaseID = BaseListLabels[setData.customGroups]

					elseif setData.label and BaseListLabels[setData.label] then
						subSet = true;
						subSetBaseID = BaseListLabels[setData.label]
					end

				--print(setData.name)
					
					if subSet then
						AddVariant(setData, subSetBaseID);
						setData.baseSetID = subSetBaseID;

					else
						BaseSets[setData.setID] = setData;

						setData.baseSetID = setData.setID;


						if setData.customGroups then
							BaseListLabels[setData.customGroups] = setData.setID;

						elseif setData.label then
							BaseListLabels[setData.label] = setData.setID;
						end

						--BaseListLabels[setData.label] = setData.setID;

						table.insert(BaseList, setData);
						AddVariant(setData, setData.setID);
			...
			--else 
				--IgnoredSets[setData.setID] = setData
		-- end
		--[[
...
function addon.GetSetType(outfitID)
	local setData = addon:GetSetByID(outfitID)
