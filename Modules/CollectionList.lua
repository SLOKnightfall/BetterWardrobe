		...
		if sources then
			...
			for i,d in pairs(sources)do
				local info = C_TransmogCollection.GetSourceInfo(d)
				 ...
				 --GetSourceInfo (called above into info) already carries isCollected; GetAppearanceSourceInfo
				 --takes a visualID, not a sourceID like d here, and no longer returns positional values anyway.
...
function CollectionList:UpdateList(type, typeID, add, sourceID)
	...
	if type == "item" then --TypeID is visualID
		...
		if type == "set" then
			...
			sources = addon.C_TransmogSets.GetSetSources(typeID)
			setName = "name"
...
function CollectionList:GenerateListView()
	...
	for i, data in ipairs(list) do
		if data then
			...
			local _, itemLink, _, _, _, _, _, _, _, itemIcon, _, _, _, _, expacID = C_Item.GetItemInfo(data.itemID)
