...
local FILTER_EXTRA_SOURCES = {L["Trash"], L["MISC"], L["Classic Set"], L["Quest Set"], L["Dungeon Set"], L["Garrison"], L["Island Expedition"], L["Warfronts"], L["Trading Post"], L["Holiday"], L["Crafted"]}
...
function WardrobeCollectionFrameMixin:SetAppearanceTooltip(contentFrame, sources, primarySourceID, warningString, slot)
	...
	--primarySourceID is often Constants.Transmog.NoTransmogID (0) until the player explicitly
	--picks a source; the tooltip's actually-displayed source is sources[self.tooltipSourceIndex].
	local displayedSource = sources and sources[self.tooltipSourceIndex or 1];
...
--Visual View TOggle;
	...
	function BetterWardrobeCollectionSavedOutfitDropdownMixin:OnLoad()
		...
		self.Popout = addon:CreateCharacterPickerPopout(dropdown, {
			staticEntries = {
		...
		self:SetScript("OnClick", function(self)
			if self.Popout:IsShown() then
				...
				self.Popout:SetPoint("TOPRIGHT", self, "BOTTOMRIGHT", 0, 0);
				self.Popout:Show();
			end
		end);
	end
	...
	function BetterWardrobeCollectionSavedOutfitDropdownMixin:OnShow()
		...
		elseif addon.SelecteSavedList == addon.SHARED_SETS_KEY then
			self:SetText("Shared Sets");
		else
			self:SetText(addon.SelecteSavedList);
		end
	end
